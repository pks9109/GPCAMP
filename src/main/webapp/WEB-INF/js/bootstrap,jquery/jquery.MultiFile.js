				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								label =	$(
										(
											'<span class="MultiFile-title">'+ '&ensp;파일 이름 : '+ n + ' | 파일 크기 : $size' +'</span>'+ 
					...
					//$.each(files, function (i, file) {
						...
						var
							...
							//2019-02-11 권수 추가
							b = $('<a id="remove" class="MultiFile-remove" style="color: red; text-decoration: none;" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
